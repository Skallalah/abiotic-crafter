...
L'étape 2 télécharge aussi les pages des **PNJ marchands** — cibles des phrases de vente relues des pages en cache — car « trading with [[The Blacksmith]] » n'a de géographie que sur la page du PNJ ({{Person}}, `appearance1..N`). Et l'étape 3 croise les infobox de **toutes** les pages de zones : les mondes-portails déclarent `enemy1..12` et `item1..12` comme les secteurs, et n'ont souvent que ça. Les créatures dont les lieux sont en prose (`== Locations ==` sans sous-titres de zone) passent par `zone_mentions`, qui cherche les noms de zones connus dans la section.
...
- Barre de catégories sous la recherche, repliée par défaut derrière une ligne de pli qui nomme la catégorie active (« ▸ Category: all ») — treize boutons mangeraient trois lignes du panneau. Dépliée : un bouton-icône par catégorie présente chez les craftables, plus « All » (le défaut) ; le pli persiste (`gate-crafting-index/catbar`) et replier ne défait pas le filtre. Icônes SVG monochromes dessinées main (le wiki n'a pas d'images de catégorie), peintes dans `--logo-ink` — noires en Windows 98, ambre en Modern Slop, comme l'arche GATE. Le filtre est additif (catégorie ∩ recherche ∩ zones découvertes), l'intertitre du groupe reste affiché en vue mono-catégorie, « Divers » ferme la rangée, et le choix persiste (`gate-crafting-index/category`, assaini contre le dataset).
...
Deux sections, dans cet ordre, calculées sur **l'arbre tel qu'il est déplié**. (La liste « crafts intermédiaires » a existé puis a été retirée : un nœud replié apparaît déjà en ressource de base avec sa ligne « à fabriquer », et l'arbre montre l'ordre — c'était un doublon.) Un nœud replié n'est pas décomposé : il compte comme un objet à se procurer entier, et apparaît dans les ressources de base avec ses moyens de l'obtenir — pour un craftable, la ligne « à fabriquer : <bench>, N composants ». Déplier ce nœud le remplace par ses composants et le fait passer en craft intermédiaire.

1. **Ressources de base** — feuilles et nœuds repliés cumulés, tri par quantité décroissante. Par ligne : icône, nom, badges, quantité (`×10` — le découpage en stacks embrouillait, la taille de stack reste dans la fenêtre d'item). Pour un dual `primary:"loot"` : ligne « ou craft : N <ingrédient> (n chacun) ». Pour un craftable **replié** : ligne « à fabriquer : <bench>, N composants » — jamais pour un dual `primary:"loot"`, qui est une feuille légitime et porte déjà sa ligne « ou craft ».
2. **Où trouver, par zone** — zones dans l'ordre de `zones[].order`. Sous chaque zone : les ressources de base qui s'y trouvent avec `where` et quantité, puis en style atténué (`.optional`) les intermédiaires duals `primary:"craft"` lootables ici, avec « optional: saves crafting N <ingrédient> ». Sources sans zone (`salvage`, `vendor`) sont regroupées sous une pseudo-zone « Autres méthodes » en dernier.
...
**Fenêtre d'un contenant ou d'une créature** : son image en grand, ses propriétés façon wiki — une par ligne, comme la fenêtre d'item. Pour tout provider : Type (la nature, « Destroyable object — break it »). Pour une créature, la fiche de l'infobox {{enemy}} de sa page complète : Type (Creature/Robot…), Codename, Origin, Identified by, Melee/Ranged (dégâts — type) — puis, côte à côte : la santé en bloc « Health » (une ligne par partie du corps, icône monochrome, nom, valeur) et les sensibilités en colonne (Weakness / Resistance / Immunity, une ligne par type de dégât, chaque type avec son icône teintée de sa couleur ; un type sans dessin retombe sur un losange en encre neutre) — champs tous optionnels, seuls ceux que le wiki connaît s'affichent (Cargo `Enemies` ne porte que `type` et `origin` ; le reste vient du wikitext, conformément au partage §4). Puis où le trouver, et son contenu — une ligne par item, du plus probable au moins probable, avec quantité et chance. Pour une créature, ce qu'elle lâche et ce qu'on récolte sur elle sont deux listes distinctes.
